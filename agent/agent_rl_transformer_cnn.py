...
from agent.dtqn_agent_cnn import DTQN_Agent_CNN
...
class Model_TrainTest:
    ...
    def train(self):
        ...
        q_value_sequence = deque(maxlen=self.sequnence_length)
        prev_q_value_sequence = deque(maxlen=self.sequnence_length)
        prev_action_sequence = deque(maxlen=self.sequnence_length)
        ...
        action = 0
        prev_q_value = torch.zeros(self.env.action_space.n)
        ...
        for episode in range(1, self.max_episodes + 1):
            ...
            while not done and not truncation:
                ...
                prev_action_sequence = add_to_sequence(prev_action_sequence, action)
                tensor_prev_action_sequence = torch.stack(list(prev_action_sequence))
                tensor_prev_action_sequence = padding_sequence_int(
                    tensor_prev_action_sequence, self.sequnence_length
                )
                tensor_prev_action_sequence = tensor_prev_action_sequence.unsqueeze(1)

                prev_q_value_sequence = add_to_sequence(
                    prev_q_value_sequence, prev_q_value
                )
                tensor_prev_q_value_sequence = torch.stack(list(prev_q_value_sequence))
                tensor_prev_q_value_sequence = padding_sequence(
                    tensor_prev_q_value_sequence, self.sequnence_length
                )

                action, _, Q_values = self.agent.select_action(
                    (
                        tensor_sequence,
                        tensor_prev_q_value_sequence,
                        tensor_prev_action_sequence,
                    )
                )
                ...
                # Q-value sequence
                q_value_sequence = add_to_sequence(q_value_sequence, Q_values)
                tensor_q_value_sequence = torch.stack(list(q_value_sequence))
                tensor_q_value_sequence = padding_sequence(
                    tensor_q_value_sequence, self.sequnence_length
                )
