...
def eval_policy(policy, env, sequence_length, device, random_map_path, render=False, max_steps=None):
    """
    ...
    for ep_num, (ep_len, ep_ret) in enumerate(
        rollout(policy, env, render, sequence_length, device, random_map_path=random_map_path, max_steps=max_steps)
    ):
