...
from agent.transformer_cnn_decoder import TransformerDQN
...
class DTQN_Agent_CNN:
    ...
    def select_action(self, state):
        ...
        if np.random.random() < self.epsilon:
            return self.action_space.sample(), None, torch.zeros(self.action_space.n)

        # Exploitation: the action is selected based on the Q-values.
        with torch.no_grad():
            # state = torch.tensor(state).to(self.device)
            # state = torch.stack(list(state))
            state, q_values, prev_actions = state
            state = state.unsqueeze(0)
            q_values = q_values.unsqueeze(0)
            prev_actions = prev_actions.unsqueeze(0)
            # print(state.shape)
            Q_values, att_weights_list = self.model((state, q_values, prev_actions))
            action = torch.argmax(Q_values[:, -1, :]).item()
            # print("Q_vals: ", Q_values)
            # print("Selected q_val: ", Q_values[:,-1,:], "Action: ", action)
            # print("att_weights_list: ", att_weights_list)
            print("Q_values: ", action, Q_values[:, -1, :])
            return action, att_weights_list, Q_values[:, -1, :].flatten()

    def learn(self, batch_size, done):

        states, prev_q_values, prev_actions, actions, q_values, next_states, rewards, dones = self.replay_memory.sample(
            batch_size
        )
        ...
        current_q_values, _ = self.model((states, q_values, actions))
        ...
        with torch.no_grad():
            future_q_values = self.target_model((next_states, q_values, actions))[0].max(dim=2, keepdim=True)[
                0
